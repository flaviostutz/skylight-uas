 ...
 */
public class Coordinates implements SerializableState {
	 ...
	 */
	public Coordinates(double latitude, double longitude, float altitude) throws IllegalArgumentException {
		setLatitude(latitude%90);
	...
	public float azimuthToRadians(Coordinates to) {
		return (float)Math.toRadians(azimuthTo(to));
	}
	...
	public void setLatitude(double latitude) throws IllegalArgumentException {
		if ((latitude < -90.0) || (latitude > 90.0))
			throw new IllegalArgumentException("Latitude must be between -90 and 90 (inclusive). value="+latitude);
	...
	public void setLongitude(double longitude) {
		if ((longitude < -180.0) || (longitude > 180.0))
			throw new IllegalArgumentException("Longitude must be between -180 and 180 (inclusive). value="+longitude);
	...
	private float distance2(Coordinates to, boolean distance)
		...
		double U1 = Math.atan((1 - Coordinates.FLATTENING) * Math.tan(phi1));
		double U2 = Math.atan((1 - Coordinates.FLATTENING) * Math.tan(phi2));
		...
		double f_a = (1.0 + Coordinates.FLATTENING)
				* Coordinates.FLATTENING / 4.0;
		double f_b = Coordinates.FLATTENING * 0.1875
		...
		for (i = 0; (i < maxIterations)
			...
			if (Double.isNaN(cos2Sigma_m)) {
				...
				if (distance)
					return (float) (Coordinates.EARTH_RADIUS * lambda);
			...
			lambda = L + (1 - C) * Coordinates.FLATTENING * sinAlpha
					* (sigma + part2);
		}
		double a2 = square(Coordinates.SEMI_MAJOR);
		double b2 = square(Coordinates.SEMI_MINOR);
		...
		if (distance) {
			double s = A * Coordinates.SEMI_MINOR * (sigma - deltaSigma);
			return (float) s;
		}
	...
	public String getFormattedLatitude() {
		String lat = getFormatted(getLatitude());
		if(getLatitude()<0) {
			lat += " S";
		} else {
			lat += " N";
	...
	public String getFormattedLongitude() {
		String lon = getFormatted(getLongitude());
		if(getLongitude()<0) {
			lon += " O";
		} else {
			lon += " L";
	...
	private String getFormatted(double value) {
		String vs = Coordinates.convert(value, Coordinates.DD_MM_SS);
		vs = vs.replace('-', ' ');
		String result = vs;
		//degrees
		int a = vs.indexOf(':');
		if(a!=-1) {
			result = vs.substring(0, a) + "°";
			
			//minutes
			int b = vs.indexOf(':', a+1);
			if(b!=-1) {
				result += vs.substring(a+1, b) + "'";
			}

			//seconds
			result += vs.substring(b+1) + "''";
	...
	public void set(Coordinates otherPosition) {
		this.setLatitude(otherPosition.getLatitude());
		this.setLongitude(otherPosition.getLongitude());
		this.setAltitude(otherPosition.getAltitude());
	}
	...
	public void setLatitudeRadians(double latitudeRadians) {
		setLatitude(Math.toDegrees(latitudeRadians));
	}
	...
	public void setLongitudeRadians(double longitudeRadians) {
		setLongitude(Math.toDegrees(longitudeRadians));
	}

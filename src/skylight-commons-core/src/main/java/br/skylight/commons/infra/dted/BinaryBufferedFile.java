...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/com/bbn/openmap/io/BinaryBufferedFile.java,v $
 ...
 * underlying input file.  The buffer size is not modifiable after
...
public class BinaryBufferedFile extends BinaryFile {
	...
	try {
		...
		if (err == -1) {
		    ...
		    if (available() == 0) {
			throw new EOFException("no bytes at all");
		    } else {
			throw new FormatException("assertSize: failed " +
						  minlength +
						  " " + bytesinbuffer + " " +
						  available());
		    }
    ...
    /**
     * closes the underlying file input, and releases some resources of the 
     ...
     * @exception IOException Any IO errors encountered in reading from
     * the file
     */

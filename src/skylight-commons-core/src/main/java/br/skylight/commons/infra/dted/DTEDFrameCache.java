 ...
 */
public class DTEDFrameCache extends CacheHandler {
	private static final Logger logger = Logger.getLogger(DTEDFrameCache.class.getName());
	
	/** The elevation value returned if there is no data at a lat/lon. */
	public final static int NO_DATA = -500;
	/**
	 * The maxumim DTED level to check for data, given a lat/lon and a path to a
	 * dted data directory.
	 */
	 ...
	 * 
	 * @param dtedPaths
	 *            path to the level 0 and level 1 dted directories
	 * @param max_size
	 *            max number of frames to keep in the cache..
	 */
	 ...
	 *            path to the level 0 and level 1 dted directories
	 * @param dted2Paths
	 *            paths to the level 2 dted directories
	 * @param max_size
	 *            max number of frames to keep in the cache..
	 */
	 ...
	 *            paths to the dted level 0 and 1 directories.
	 * @param paths2
	 *            paths to the dted level 2 directories.
	 */
	 ...
	 * 
	 * @param searchPaths
	 *            an array of dted root directories
	 * @param partialFile
	 *            the relative pathname of a dted frame file from the dted root.
...
//			frame.initSubframes(numXSubframes, numYSubframes);
	 ...
	 * 
	 * @param max_size
	 *            the maximum number of frames in the cache.
	 * @param num_x_subframes
	 *            the number of horizontal subframes in each frame.
	 * @param num_y_subframes
	 *            the number of vertical subframes in each frame.
	 */
	public void resizeCache(int max_size, int num_x_subframes, int num_y_subframes) {
		...
		if (num_x_subframes > 0 && num_y_subframes > 0) {
			...
			destructive = true;
			logger.fine("DTEDFrameCache: destructive resizing");
		} else {
			logger.fine("DTEDFrameCache: passive resizing");
		}
		...
		for (int i = 0; i < objs.length; i++) {
			...
			if (frame == null) {
				logger.fine("DTEDFrameCache: No Frame for key!");
				continue;
			}
...
//				frame.initSubframes(num_x_subframes, num_y_subframes);
	 ...
	 * 
	 * @param proj
	 *            the projection describing the wanted area
	 * @param dtedLevel
...
//
//		return getElevations(ul.getLatitude(), ul.getLongitude(), lr.getLatitude(), lr.getLongitude(), dtedLevel);
//	}
	...
	protected short[][] getElevations(float ullat, float ullon, float lrlat, float lrlon, int dtedLevel, DTEDFrame refFrame) {
		...
		if (ullon > lrlon) {
			if (ullon > 0 && lrlon < 0) {
				logger.warning("DTEDFrameCache: getElevations: Stradling dateline not handled!");
				return null;
			}
			right = ullon;
			left = lrlon;
		...
		for (x = 0; x < xSize; x++) {
			...
			for (y = 0; y < ySize; y++) {
				...
				if (thisFrame != null) {
					...
					if (refFrame != null) {
						logger.fine("DTEDFrameCache: Missing frames, going to use reference frame");
...
//		String[] paths2 = PropUtils.initPathsFromProperties(props, prefix + DTED2PathsProperty);
//

...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/com/bbn/openmap/layer/dted/DTEDFrameDSI.java,v $
...
public class DTEDFrameDSI {
	...
	private static final Logger logger = Logger.getLogger(DTEDFrameDSI.class.getName());
	...
	public DTEDFrameDSI(BinaryFile binFile) {
		...
		try {
			...
			try {
				...
				logger.info("DTEDFrameDSI: Data Edition number bad, using 0");
				data_ed = 0;
			}
			...
			// spec_amen = Integer.parseInt(binFile.readFixedLengthString(2),
			// 10);
			try {
				test = binFile.readFixedLengthString(2);
				spec_amen = Integer.parseInt(test, 10);
			} catch (NumberFormatException nfe) {
				logger.info("DTEDFrameDSI: Spec Amednment number bad, using 0");
				spec_amen = 0;
			}
			...
			try {
				...
				logger.info("DTEDFrameDSI: orient angle number bad, using 0");
				orient_ang = 0f;
			}
			...
			// part_cell = Integer.parseInt(binFile.readFixedLengthString(2),
			// 10);
			try {
				test = binFile.readFixedLengthString(2);
				part_cell = Integer.parseInt(test, 10);
			} catch (NumberFormatException nfe) {
				logger.info("DTEDFrameDSI: partial cell number bad, using 0");
				part_cell = 0;
			}
		} catch (IOException e) {
			logger.severe("DTEDFrameDSI: File IO Error!\n" + e.toString());
		} catch (FormatException f) {
			logger.severe("DTEDFrameDSI: File IO Format error!\n" + f.toString());
		} catch (NumberFormatException nfe) {
			...
			// filled in.
			logger.severe("DTEDFrameDSI: Number format error!\n" + nfe.toString());
		}
	}

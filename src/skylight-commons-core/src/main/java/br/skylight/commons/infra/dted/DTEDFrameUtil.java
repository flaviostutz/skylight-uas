...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/com/bbn/openmap/layer/dted/DTEDFrameUtil.java,v $
// $RCSfile: DTEDFrameUtil.java,v $
// $Revision: 1.8 $
// $Date: 2002/04/02 21:07:51 $
// $Author: bmackiew $
// 
// **********************************************************************
...
public class DTEDFrameUtil {
     ...
     *
     * @param text_lat the String representation of the latitude, from the file.
    ...
    public static float stringToLat(String text_lat){
        ...
        if (text_lat.length() > 7 && text_lat.charAt(6) != '.') offset = 1;
        ...
        if (text_lat.charAt(6+offset) == '.'){
            ...
            if (text_lat.charAt(8+offset) == 'S') hemisphere = -1;
        }
        else {
            s_seconds = String.valueOf(contents, 4+offset, 2);
            if (text_lat.charAt(6+offset) == 'S') hemisphere = -1;
        }
     ...
     *
     * @param text_lon the longitude String representation, out of a DTED file.
    ...
    public static float stringToLon(String text_lon){
        ...
        if (text_lon.charAt(7) == '.'){
            ...
            if (text_lon.charAt(9) == 'W') hemisphere = -1;
        }
        else {
            s_seconds = String.valueOf(contents, 5, 2);
            if (text_lon.charAt(7) == 'W') hemisphere = -1;
        }
     ...
     * filename.  DTED filenames are based on these two factors -
     * <n/s><value>.dt{0,1} DTED Level 0 files are named .dt0. Level 1 and

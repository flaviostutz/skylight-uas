...
// **********************************************************************
// 
// $Source: /cvs/distapps/openmap/com/bbn/openmap/io/StreamInputReader.java,v $
// $RCSfile: StreamInputReader.java,v $
// $Revision: 1.3.2.1 $
// $Date: 2002/07/17 21:21:38 $
// $Author: dietrick $
// 
// **********************************************************************
...
public abstract class StreamInputReader implements InputReader {
	...
	private static final Logger logger = Logger.getLogger(StreamInputReader.class.getName());
	 ...
	 * 
	 * @return number of bytes added in this call, to pass along to anything
	 *         else that will be interested.
	 */
	...
	protected void reopen() throws IOException {
		if (inputStream != null) {
			logger.fine("StreamInputReader: Closing inputStream");
			inputStream.close();
		}
	...
	public long skipBytes(long n) throws IOException {
		...
		while (count < n) {
			...
			if (gotsofar == 0) { // added from david marklund
				logger.severe("StreamInputReader can't skip " + n + " bytes as instructed");
				break;
			}
			count += gotsofar;
		}
	...
	public void seek(long pos) throws IOException {
		...
		if (pos >= curPosition) {
			if (seekComments) {
				logger.fine("StreamInputReader - seeking to " + pos + " from " + curPosition);
			}

			skipped = skipBytes(pos - curPosition);

			if (seekComments) {
				logger.fine("   now at: " + inputStreamCount + ", having skipped " + skipped);
			}
		} else {
			if (seekComments) {
				logger.fine("StreamInputReader - having to start over for seek - " + pos + " from " + curPosition);
			}
			reopen();
			if (seekComments)
				logger.fine("   skipping to: " + pos);
	...
	public void close() throws IOException {
		try {
			logger.fine("StreamInputReader.close()");
			if (inputStream != null)
				inputStream.close();
		} catch (Exception e) {
			e.printStackTrace();

...
public class ControlRelinquishDialog extends JDialog {
	...
	private List<ControlRelinquishItem> requestItems = new ArrayList<ControlRelinquishItem>();  //  @jve:decl-index=0:
	 ...
	 * @param owner
	 */
	public ControlRelinquishDialog(Frame owner, ControllableElement element, MessagingService 	messagingService, UserService userService) {
		super(owner);
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabel4 = new JLabel();
			jLabel4.setText("Relinquish control to the following elements:");
			...
			if(element instanceof Vehicle) {
				...
				//VEHICLES
				addElementsForControllableElement(v);

				//PAYLOADS
				for (Payload p : v.getPayloads().values()) {
					addElementsForControllableElement(p);
				}
	...
	private void addElementsForControllableElement(ControllableElement ce) {
		CUCSControl cc = ce.getCucsControls().get(userService.getCurrentCucsId());
		if(cc!=null) {
			for (LOI loi : cc.getGrantedLOIs().getLOIs()) {
				ControlRelinquishItem ci = new ControlRelinquishItem(ce, loi);
	...
	private JButton getRequestButton() {
		if (requestButton == null) {
			...
			requestButton.setText("Relinquish control");
			requestButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					for (ControlRelinquishItem ri : requestItems) {
						if(ri.getCheckbox().isSelected()) {
							...
							//LOI
							m.setRequestedHandoverLOI(BitmappedLOI.valueOf(ri.getLoi()));
							m.setControlledStationMode(ControlledStationMode.RELINQUISH_CONTROL);
